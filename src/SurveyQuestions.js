...
const SurveyQuestions = ({currentPage, set_id, answers, setAnswers, allQuestions, currentQuestions, setCurrentQuestions, answeredCount, textResponses, setTextResponses, totalPages, onPageChange, onDataLoad, userUUID, host_development, questionsPerPage}) => {
...
/*  useEffect(() => {
  ...
  useEffect(() => {
    if (allQuestions && allQuestions.length > 0) {
        ...
        console.log("before page change: pg:",prevPage, prevQ[0]?.category, prevQ[0]?.num, optionLength );
        let advance = 0;
        ...
        console.log("page change: currentPage:",currentPage, 'start',start, 'adv',advance, qlist[0].category, qlist[0].num, JSON.parse(qlist[0].valid)?.length);
        setCurrentQuestions(qlist);
    }
  }, [currentPage, allQuestions, answers, questionsPerPage , setCurrentQuestions ]); // Recalculate displayed questions when currentPage or allQuestions changes


    const handleNext = (advance_count = 1) => {
        //console.log("set next page current Page",currentPage, "total Pages", totalPages);
        onPageChange(currentPage => Math.min(currentPage + advance_count, totalPages));
  ...
  const submitAnswerRadio = async (set_id, question_id, answer_id) => {
    try {
  console.log(" submitAnswerRadio Q:",question_id," A:",answer_id, "set",set_id, userUUID);
  ...
  const submitAnswerOtherText = async (set_id, question_id, answer_other) => {
    try {
      ...
      console.error(error, 'Error submitting set_id:', set_id, 'question_id:',question_id, 'other answer:', answer_other);
    }
  };
  ...
  const handleAnswerChange = (set_id, question_id, answer_id) => {
    ...
    console.log("handleAnswerChange for question", question_id, "A",answer_id, "cat",category_id);
    setAnswers(prevAnswers => {
        return {
            ...prevAnswers,
            [question_id]: {set_id:set_id, question_id:question_id, answer_id:answer_id, category:category_id}
        }
    });
  ...
  const onTextOtherChange = (set_id, questionId, answer) => {
   ...
   const handleTextOtherChange = (set_id, question_id, text) => {
     ...
     setTextResponses(prevAnswers => {
       const exists = prevAnswers.some(answer => answer.question_id === question_id);
       let newAnswers;
       if (exists) {
         newAnswers = prevAnswers.map(answer => 
           answer.question_id === question_id ? {...answer, answer_other: text} : answer
          );
        } else {
            //add new answer
            newAnswers = [...prevAnswers, {question_id: question_id, answer_other: text}];
        }
   ...
   const bg_style = {
    background: 'linear-gradient(to bottom right, #FFB347, #E6E6FA)',
    width: '100%',
    height: '100vh',
  };
  return (
      ...
      <div className="main-content" style={bg_style}>
      ...
      <div className="button-container">
       ...
       <div className="buttons-right">
         <button className="button" onClick={(e) => { e.preventDefault(); handlePrevious(); }}>
           <svg width="40" height="20" viewBox="0 0 40 20">
            <path d="M20 0l20 20H0L20 0z" fill="white" />
          </svg>
        </button>
        <button className="button" onClick={(e) => { e.preventDefault(); handleNext(); }}>
         <svg width="40" height="20" viewBox="0 0 40 20">
          ...
          <button
            ...
            onClick={(e) => { e.preventDefault(); handleResults(); }}
          >
            Answers | Results
          </button>
        </div>
